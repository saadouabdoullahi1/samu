...
export default function SearchPanel() {
  ...
  const search = async () => {
    const cat = selectedCategory && selectedCategory !== "Other" ? selectedCategory : "";
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex flex-col gap-6 rounded-3xl border border-stone-200 bg-white p-6 shadow-sm">
        ...
        <div className="flex flex-col gap-2">
          <div className="relative">
            ...
            <textarea
              ...
              placeholder="Describe your item… e.g. black Samsung phone with a blue case, lost near Yantala"
              rows={3}
          ...
          {hasUnderstanding && (
            <div className="flex flex-wrap items-center gap-2 rounded-2xl bg-brand-soft/60 px-4 py-3">
              <span className="text-sm font-medium text-brand-dark">Samu understood:</span>
              {criteria.category && (
                <Chip>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="flex flex-col gap-2">
            <span className="flex items-center gap-1.5 text-sm font-medium text-stone-700">
              <MapPin className="h-4 w-4" /> Where did you lose it?
            </span>
            <input
              value={zone}
              onChange={(e) => setZone(e.target.value)}
              placeholder={criteria.zone ? criteria.zone : "e.g. Yantala"}
        ...
        >
          <Search className="h-5 w-5" /> {searching ? "Searching…" : "Search"}
        </button>
      </div>
      ...
      <div className="flex items-start gap-3 rounded-2xl bg-stone-100 px-4 py-3 text-sm text-stone-600">
        ...
        <p>
          <strong className="text-stone-800">Private search.</strong> The details used to verify that
          you are the owner are never shown in public results.
        </p>
      </div>
      ...
      {matches !== null && matches.length > 0 && (
        <div className="flex flex-col gap-5">
          <h2 className="text-xl font-bold">
            {matches.length} item{matches.length > 1 ? "s" : ""} could match
          </h2>
      ...
      {matches !== null && matches.length === 0 && (
        <div className="flex flex-col items-center gap-4 rounded-3xl border border-stone-200 bg-white px-6 py-14 text-center">
          ...
          <div className="flex flex-col gap-1">
            <h2 className="text-xl font-bold">No item found</h2>
            <p className="max-w-sm text-stone-600">
              We don&apos;t have a matching item yet. It may be reported later.
            </p>
          </div>
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            >
              <Bell className="h-4 w-4" /> Create an alert
            </button>
            <button
              type="button"
          ...
          {alerted && (
            <p className="flex items-center gap-1.5 text-sm text-brand-dark">
              <Bell className="h-4 w-4" /> Coming soon: Samu will notify you as soon as a matching item
              is reported.
            </p>
          )}
        </div>
